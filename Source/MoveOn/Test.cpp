...
FString UTest::GetPointlessMassage()
{
	return FString::Printf(TEXT("This message is pointless"));
}

double UTest::GetMyPi()
{
	return Pi;
}

FAverages UTest::CalculateAverages(const TArray<int32>& InValues)
{
	...
	? TmpArray[Count / 2]
	: static_cast<float>((ModalCount[Count / 2 - 1] + TmpArray[Count / 2 - 1])) / 2;


	ModalCount.ValueSort([](int a, int b) { return a > b; });
